...
class DrawWaypoints:
    ...
    def __init__(self, waypoints: MsgWaypoints, radius: float, color: NP_MAT, window: gl.GLViewWidget) -> None:
        ...
        if waypoints.type in ('straight_line', 'fillet'):
            ...
            (points, color) = dubins_points(waypoints, self.radius, 0.1)
        self.color = color
    ...
    def update(self, waypoints: MsgWaypoints) -> None:
        ...
        if waypoints.type in ('straight_line', 'fillet'):
            ...
            (points, colors) = dubins_points(waypoints, self.radius, 0.1)
        self.waypoint_plot_object.setData(pos=points, color=colors)

def dubins_points(waypoints: MsgWaypoints, radius: float, Del: float) -> tuple[NP_MAT, NP_MAT]:
    """
    ...
    """
    # DEFINE COLORS
    color_arc1 = np.array([[146, 255, 161, 255]])/255.
    color_arc2 = np.array([[30, 144, 255, 255]])/255.
    color_line = np.array([[255, 152, 255, 255]])/255.
    color_arc3 = np.array([[255, 152, 71, 255]])/255.
    ...
    for j in range(0, waypoints.num_waypoints-1):
        ...
        for angle in theta_list:
            ...
            colors = np.concatenate((colors, color_arc1), axis=0)

        
        # points along the intermediate circle
        if dubins_path.aaa is True:
            # points along start circle
            th1 = np.arctan2(dubins_path.r1.item(1) - dubins_path.center_i.item(1),
                                dubins_path.r1.item(0) - dubins_path.center_i.item(0))
            th1 = mod(th1)
            th2 = np.arctan2(dubins_path.r2.item(1) - dubins_path.center_i.item(1),
                                dubins_path.r2.item(0) - dubins_path.center_i.item(0))
            th2 = mod(th2)
            th = th1
            theta_list = [th]
            if dubins_path.dir_i > 0:
                if th1 >= th2:
                    while th < th2 + 2*np.pi:
                        th += Del
                        theta_list.append(th)
            ...
            if initialize_points:
                points = np.array([[dubins_path.center_i.item(0) + dubins_path.radius * np.cos(theta_list[0]),
                                    dubins_path.center_i.item(1) + dubins_path.radius * np.sin(theta_list[0]),
                                    dubins_path.center_i.item(2)]])
                colors = color_arc2
                initialize_points = False
            for angle in theta_list:
                new_point = np.array([[dubins_path.center_i.item(0) + dubins_path.radius * np.cos(angle),
                                        dubins_path.center_i.item(1) + dubins_path.radius * np.sin(angle),
                                        dubins_path.center_i.item(2)]])
                points = np.concatenate((points, new_point), axis=0)
                colors = np.concatenate((colors, color_arc2), axis=0)
        ...
        # points along straight line
            ...
            while sig <= 1:
                ...
                points = np.concatenate((points, new_point), axis=0)
                colors = np.concatenate((colors, color_line), axis=0)
                sig += Del
        ...
        for angle in theta_list:
            ...
            points = np.concatenate((points, new_point), axis=0)
            colors = np.concatenate((colors, color_arc3), axis=0)
