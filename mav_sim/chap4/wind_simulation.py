...
class WindSimulation:
    ...
    def __init__(self, Ts: float, gust_params: Optional[MsgGustParams] = None, gust_flag: bool = True) -> None:
        ...
        a1 = a2 * Va / (np.sqrt(3) * Lv)
        b1 = Va / Lv

        self.v_w = TransferFunction(num=np.array([[a2, a1]]),
                                     den=np.array([np.convolve([1, b1], [1, b1])]),
                                     Ts=Ts)

        a2 = sigma_w * np.sqrt(3. * Va / (np.pi * Lw))
        a1 = a2 * Va / (np.sqrt(3) * Lw)
        b1 = Va / Lw
        self.w_w = TransferFunction(num=np.array([[a2, a1]]),
                                     den=np.array([np.convolve([1, b1], [1, b1])]),
                                     Ts=Ts)
        self._Ts = Ts
